...
print_aggregated_failsafe_summaries() {
   ...
   add_to_report "     </modules>"
   local end_message="Total number of executed tests is $completed while $skipped tests were skipped for combination '$command_mvn_args', git branch '$git_branch'"
...
generate_stats_for_branch() {
    ...
    do
      ...
      print_aggregated_failsafe_summaries "$enhanced_mvn_args" "$target_branch"
    done
}
...
RECIPE_PROJECT_URL='https://github.com/michalvavrik/quarkus-utilities.git'
